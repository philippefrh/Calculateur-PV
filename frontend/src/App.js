  ...
  return (
    <div className="form-container">
      ...
      <form onSubmit={handleSubmit}>
        ...
        {/* Section d'analyse de toiture */}
        <div className="roof-analysis-section">
          {!showRoofAnalysis ? (
            <div className="roof-analysis-toggle">
              <button 
                type="button" 
                className="roof-analysis-button"
                onClick={() => setShowRoofAnalysis(true)}
              >
                📸 Insérer photos de la toiture
              </button>
              <small>Visualisez le placement optimal des panneaux solaires sur votre toiture</small>
            </div>
          ) : (
            <div className="roof-analysis-panel">
              <div className="roof-analysis-header">
                <h4>🏠 Analyse de votre toiture</h4>
                <button 
                  type="button" 
                  className="close-analysis-button"
                  onClick={() => {
                    setShowRoofAnalysis(false);
                    setRoofImage(null);
                    setRoofAnalysisResult(null);
                  }}
                >
                  ✕ Fermer
                </button>
                  ...
                  <div className="upload-area">
                    <label htmlFor="roof-image-upload" className="upload-label">
                      ...
                      <div className="upload-text">
                        <strong>Cliquez pour sélectionner une photo de votre toiture</strong>
                        <br />
                        <small>Formats acceptés: JPG, PNG, WEBP (max 10MB)</small>
                      </div>
                    </label>
                    <input 
                      ...
                      type="file" 
                      accept="image/*" 
                      onChange={handleRoofImageUpload}
                      style={{ display: 'none' }}
                    />
                  </div>
                  <div className="upload-tips">
                    <h5>💡 Conseils pour une meilleure analyse :</h5>
                    <ul>
                      <li>Photo prise de face ou légèrement en hauteur</li>
                      <li>Toiture bien visible et éclairée</li>
                      <li>Évitez les photos prises par mauvais temps</li>
                    </ul>
                  </div>
                </div>
              ) : (
                <div className="image-analysis-section">
                  ...
                  {!roofAnalysisResult ? (
                    <div className="analysis-controls">
                      ...
                      >
                        {isAnalyzingRoof ? '🔄 Analyse en cours...' : '🤖 Analyser avec IA'}
                      </button>
                      <button 
                        type="button" 
                    ...
                    <div className="analysis-results">
                      ...
                      <div className="analysis-summary">
                        <p><strong>Placement possible:</strong> {roofAnalysisResult.placement_possible ? '✅ Oui' : '❌ Non'}</p>
                        <p><strong>Surface requise:</strong> {roofAnalysisResult.total_surface_required} m²</p>
                        <p><strong>Nombre de panneaux:</strong> {roofAnalysisResult.panel_positions.length}</p>
                      </div>
                      ...
                      <div className="analysis-details">
                        <p><strong>Analyse:</strong> {roofAnalysisResult.roof_analysis}</p>
                        <p><strong>Recommandations:</strong> {roofAnalysisResult.recommendations}</p>
                      </div>
...
function App() {
  ...
  // Fonctions pour l'analyse de toiture
  const handleRoofImageUpload = (event) => {
  ...
  const analyzeRoofWithAI = async () => {
    if (!roofImage) {
      alert('Veuillez sélectionner une image de toiture');
      return;
    }
    
    // Déterminer le nombre de panneaux selon le kit sélectionné ou recommandé
    let panelCount = 6; // Par défaut
    if (formData.useManualKit && formData.manualKit) {
      panelCount = formData.manualKit.panels;
    } else {
      // Estimation basée sur la consommation
      const consumption = parseInt(formData.annualConsumption) || 6000;
      if (consumption < 4000) panelCount = 6;
      else if (consumption < 8000) panelCount = 12; 
      else panelCount = 18;
    }
    ...
    try {
      ...
      const base64Data = roofImage.base64.split(',')[1];
      ...
      if (response.data.success) {
        setRoofAnalysisResult(response.data);
      } else {
        alert('Erreur lors de l\'analyse: ' + response.data.roof_analysis);
      }
    } catch (error) {
      console.error('Erreur analyse toiture:', error);
      alert('Erreur lors de l\'analyse de la toiture. Veuillez réessayer.');
    } finally {
