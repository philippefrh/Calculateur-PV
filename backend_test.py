...
BACKEND_URL = "https://53e569bc-dd7c-4cc2-8ba6-7536f9032dc9.preview.emergentagent.com/api"
...
class SolarCalculatorTester:
    ...
    def test_roof_image_upload(self):
        ...
        try:
            ...
            # Create a 400x300 test roof image
    ...
    def test_roof_visualization_error_handling(self):
        """Test error handling for roof visualization endpoints"""
        try:
            test_cases = []
            ...
            response = self.session.post(f"{self.base_url}/generate-roof-visualization", json=invalid_request)
            if response.status_code == 200:
                data = response.json()
                if not data.get("success") and "Invalid image format" in data.get("error_message", ""):
                    test_cases.append("✅ Invalid image format handled correctly")
                else:
                    test_cases.append("❌ Invalid image format not handled properly")
            else:
                test_cases.append(f"❌ Invalid image format test: HTTP {response.status_code}")
            
            # Test 2: Invalid kit power for France
            if hasattr(self, 'test_image_data'):
                invalid_power_request = {
                    "image_data": self.test_image_data,
                    "kit_power": 15,  # Not valid for France
                    "region": "france"
                }
                
                response = self.session.post(f"{self.base_url}/generate-roof-visualization", json=invalid_power_request)
                if response.status_code == 200:
                    data = response.json()
                    if not data.get("success") and "Invalid kit power" in data.get("error_message", ""):
                        test_cases.append("✅ Invalid kit power handled correctly")
                    else:
                        test_cases.append("❌ Invalid kit power not handled properly")
                else:
                    test_cases.append(f"❌ Invalid kit power test: HTTP {response.status_code}")
            ...
            if hasattr(self, 'test_image_data'):
                ...
                response = self.session.post(f"{self.base_url}/generate-roof-visualization", json=invalid_region_request)
                if response.status_code == 200:
                    data = response.json()
                    if not data.get("success"):
                        test_cases.append("✅ Invalid region handled correctly")
                    else:
                        test_cases.append("❌ Invalid region not handled properly")
                else:
                    test_cases.append(f"❌ Invalid region test: HTTP {response.status_code}")
            
            # Test 4: File too large for upload
            large_file_data = b'x' * (11 * 1024 * 1024)  # 11MB (over 10MB limit)
            files = {'file': ('large_roof.jpg', large_file_data, 'image/jpeg')}
            
            response = self.session.post(f"{self.base_url}/upload-roof-image", files=files)
            if response.status_code == 200:
                data = response.json()
                if not data.get("success") and "too large" in data.get("error_message", ""):
                    test_cases.append("✅ Large file size handled correctly")
                else:
                    test_cases.append("❌ Large file size not handled properly")
            else:
                test_cases.append(f"❌ Large file test: HTTP {response.status_code}")
            
            # Test 5: Non-image file upload
            files = {'file': ('test.txt', b'This is not an image', 'text/plain')}
            
            response = self.session.post(f"{self.base_url}/upload-roof-image", files=files)
            if response.status_code == 200:
                data = response.json()
                if not data.get("success") and "must be an image" in data.get("error_message", ""):
                    test_cases.append("✅ Non-image file handled correctly")
                else:
                    test_cases.append("❌ Non-image file not handled properly")
            else:
                test_cases.append(f"❌ Non-image file test: HTTP {response.status_code}")
            
            # Evaluate results
            failed_tests = [t for t in test_cases if t.startswith("❌")]
            
            if not failed_tests:
                self.log_test("Roof Visualization Error Handling", True, 
                            f"✅ All error handling tests passed: {'; '.join(test_cases)}", 
                            {"total_tests": len(test_cases), "passed": len(test_cases)})
            else:
                self.log_test("Roof Visualization Error Handling", False, 
                            f"Some error handling tests failed: {'; '.join(test_cases)}", 
                            {"total_tests": len(test_cases), "failed": len(failed_tests)})
                
        except Exception as e:
            self.log_test("Roof Visualization Error Handling", False, f"Error: {str(e)}")
    ...
    def test_fal_ai_integration_and_black_panels(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Verify OmniGen V2 model usage (check backend code implementation)
                # This is verified by the successful generation and proper URL format
                ...
                # Note: We can't directly verify BLACK color from the API response,
                # but the backend code shows the prompt specifically requests "black solar panels"
                # and "Modern matte black finish (like Powernity 375W panels)"
